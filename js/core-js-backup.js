...
document.body.addEventListener("dblclick", removeAllSelects);
...
function setElementsToEdit() {
	...
	setElement(".row");
	setElement(".column");
	setElement(".accordion-container");
	setElement(".datas__columns");
	setElement(".lista");
	setElement(".tabs");
	setElement(".datas__row");
	// Tabela
	setElement("tr");
	setElement("td");
	setElement("th");
...
function setElement(type) {
	// - encontrar todos paragrafos dentro de content
	var getAllType = document.getElementById("content").querySelectorAll(type);

	// - adicionar funcao de duplicar paragrafo evento de doubleclick
	for (i = 0; i < getAllType.length; i++) {
  	  getAllType[i].addEventListener("click", addSelection);
	  getAllType[i].addEventListener("dblclick", removeSelection);
	}
}

	
// Adiciona uma classe de selecionado ao elemento clicado
...
function KeyPress(e) {
	  ...
	  if (evtobj.ctrlKey && evtobj.shiftKey && evtobj.keyCode == 90) {
	    nextHistory();
	  } else if (evtobj.ctrlKey && evtobj.keyCode == 90) {
	     previousHistory();
	  }
